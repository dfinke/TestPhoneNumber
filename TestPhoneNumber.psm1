...
function Test-PhoneNumber {
    ...
    $ValidNAAreaCodes = '201', '202', '203', '204', '205', '206', '207', '208',
    ...
    if ($PhoneNumber.Length -eq 11) {
        if ($PhoneNumber.Substring(0, 1) -ne 1) {
            return $false
        }
    ...
    if ($PhoneNumber.Length -ne 10) {return $false}

    $areaCode = $PhoneNumber.Substring(0, 3)
    $exchangeCode = $PhoneNumber.Substring(3, 3)
    $stationCode = $PhoneNumber.Substring(6)

    if ($areaCode -notin $ValidNAAreaCodes) {
        return $false
    }

    if ($exchangeCode[1] -eq '1' -and $exchangeCode[2] -eq '1') {
        return $false
    }

    if ($exchangeCode -eq '555') {
        if ($stationCode -eq '1212' -Or $stationCode -eq '4334') {
            return $false
        }

        if ($stationCode[0] -eq '0') {
            [int]$stationCodeNumber = $stationCode
            if ($stationCodeNumber -ge 100 -and $stationCodeNumber -le 199) {
                return $false
            }
        }        
    }
